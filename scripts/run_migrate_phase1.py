"""
Run sql/migrate_phase1_youtube_titans.sql (YouTube stats columns + TITANS support).
Requires DATABASE_URL. Safe to run multiple times (ADD COLUMN IF NOT EXISTS).
Usage: python scripts/run_migrate_phase1.py
"""
...
def main() -> int:
    ...
    if not db_url:
        ...
        return 1
    path = ROOT / "sql" / "migrate_phase1_youtube_titans.sql"
    if not path.exists():
